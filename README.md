...
1504. Seeding also publishes 10 `plant.NNN.*` copies that reuse the original
three Modbus connections and point each copy at that plant's register block.
Set `SIMULATOR_REPLICAS` to match `SIMULATOR_INSTANCES`, or `0` to seed only
the operator plant. See `simulator/README.md` for the register map, controls,
fault scenarios, and verification commands.
